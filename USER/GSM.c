...
unsigned char  AT_QMDA[] = "AT+QMGDA= DEL ALL ";
...
unsigned char  AT_CMGD[]	=	"AT+CMGD=";//Send SMS command
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
///Function void receive (void) interrupt 4 using 1; achieve functions: receiving data through the serial port////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/******************************************************************************************************************/
//void receive(void)
//{
...
/******************************************************************************************************************/
/*u8 check_exit_key(void)
{   unsigned char flag_gpio = 1,digit[4],set_password_fill_flag ,password_correct_flag= 0 ;
		unsigned char menu_level_trace =0x00,clear_flag;
		uint32_t temp_value=0,temp_value2 =0 ,i=0,temp_var;unsigned char key_trace_var ;
		unsigned int password = 0xf6f3, temp_password = 0;unsigned char chk_key;
		clear_flag =1;
		
		chk_key = key_check();
		if(chk_key == EXIT)
		{lcd_clr();delay(0xFFF);	lcd_gotoxy(1,1);delay(0xFFF);
										lcd_write_string("Enter password");	delay(0xFFF);lcd_gotoxy(2,1);
										flag_gpio = 0;set_password_fill_flag = 0;
										temp_value2 = *(__IO uint32_t*)password_START_ADDR;
										while( 1)
										{		key_trace_var = key_check();
												delay(0xFFF);
												if(key_trace_var == key_check())
												{		
														if(key_trace_var <= 0x39 && key_trace_var >= 0x30 && set_password_fill_flag == 0)
														{
																if(flag_gpio == 1 && set_password_fill_flag == 0)
																{						
																flag_gpio = 0;
																send_data(key_trace_var);
																digit[i] = key_trace_var ;  
																i++;
																		if(i > 3)
																		{		
																					set_password_fill_flag = 1;
																					lcd_clr();
																					delay(0xFFF);
																					lcd_gotoxy(1,1);delay(0xFFF);
																					flag_gpio = 0;
																					temp_password = 1000*digit[3] + 100*digit[2] + 10*digit[1] + digit[0];
																					temp_value = digit[0];temp_value = temp_value<<8;
																					temp_value = temp_value|digit[1];temp_value = temp_value<<8;
																					temp_value = temp_value|digit[2];temp_value = temp_value<<8;
																					temp_value = temp_value|digit[3];
																					delay(0xFFFF);
																					if(password == temp_password || temp_value == temp_value2)
																					{
																						lcd_write_string("ACCESS GRANTED");
																						delay_key(0xFFFFFF);delay_key(0xFFFFF);
																						password_correct_flag = 0;
																						set_password_fill_flag = 1;
																						i = 0;sendstring(ATH);
																						lcd_clr();delay(0xFFF);
							lcd_gotoxy(1,1);delay(0xFFF);flag_gpio =0;
							lcd_write_string("FULL DISARM");delay(0xFFF);
							menu_level_trace = 0x02;
					...
					BKP_WriteBackupRegister(BKP_DR9,temp_var );	
										...
										delay_key(0xFFFFFF);delay_key(0xFFFFF);
																		...
																		{
																		delay_key(0xFFF);
																		set_password_fill_flag = 0;
																		}					
																}				*/
																
													//		}//while(key_check() == 0x00)		
												/*			if(keyno == EXIT)
															{	flag_gpio = 0;	
																return ;
															}*/
...
/******************************************************************************************************************/ 
 void message_read(void)
{		unsigned char tem_var,pos=0,counter = 1,j[4],i;
		uint32_t  message_pos = 0,password_temp;

		while(pos<75)
		{
			if((SystemBuf[pos]==0x0D)&&(SystemBuf[++pos]==0x0A))
			{	
			if(counter == 2)
				{	pos++;
					message_pos = pos +5; 
		...
		//POS :POSITION FOR MESSGE START PASSWORD
		//check password code of sms
		FLASH_Unlock();/* Clear All pending flags */
		FLASH_ClearFlag(FLASH_FLAG_EOP | FLASH_FLAG_PGERR | FLASH_FLAG_WRPRTERR);	
		password_temp =(*(__IO uint32_t*)password_START_ADDR);

		j[0] = (uint8_t)((password_temp & 0xFF000000)>>24);
		j[1] = (uint8_t)((password_temp & 0x00FF0000)>>16);
		j[2] = (uint8_t)((password_temp & 0x0000FF00)>>8);
		j[3] = (uint8_t)(password_temp  & 0x000000FF);
	if((SystemBuf[pos] 	 == j[0])&&
	(SystemBuf[++pos] == j[1])&&
	(SystemBuf[++pos] == j[2])&&
	(SystemBuf[++pos] == j[3]))
		{//password correct , now save number in buffer
				pos =0;counter=0;
				while(pos<50)
				{
					if(SystemBuf[pos]==0x22)
					{	counter++;
						if(counter == 3)
						{ pos++;
							break;//search position of number 
						}						
					}
				pos++;
				}// save nuMber in NUMBER buffer
				while(i<14&&(SystemBuf[pos]!=0x22))
				{
				numberbuf[i] = SystemBuf[pos];
				i++;pos++;numberbuf[i]=0x03;
				}
				//number saved in buffer
				tem_var = SystemBuf[message_pos]; 
				readcommend(tem_var,message_pos);
				delete_message(1);delete_message(2);
		}
		else
		{//delete message
			delete_message(1);
